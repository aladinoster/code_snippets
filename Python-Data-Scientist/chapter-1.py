...
num_friend_by_id_sorted = sorted(num_friends_by_id,  # get it sorted
                                 key=lambda friends_by_id: friends_by_id[1], reverse=True)

# Recomending friends


def friends_of_friends_ids_bad(user):
    return [foaf["id"] for friend in user["friends"]  # for each of user's f
...
print("Firends of friends:", friends_of_friends_ids(users[3]))
...
# Way to find users based interests
...
# Former implementation is to hard better to add keys to a dictionary
...
def most_common_interests_with(user):
    return Counter(
        interested_user_id for interest in interests_by_user_id[user["id"]] for interested_user_id in user_ids_by_interest[interest] if interested_user_id != user["id"])


print(f"Most common interests with {users[0]['id']}:",
...
# Former result does not aggregate, to aggregate by buckets.
